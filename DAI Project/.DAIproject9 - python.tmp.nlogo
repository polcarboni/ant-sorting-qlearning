...
to setup
  ...
  ifelse leader-learning [set leader-learning-const 1] [set leader-learning-const 0]
  ...
  ask patches [
    if ( random ( ( world-width ^ 2 ) / ( objects-number * 4) ) ) < 1
      [ set pcolor ( random colors ) * randomize-colors * 100 + orange + 0.5]

    set objects-number-const objects-number
...
;;_____________________________ANTS_ACTIONS______________________________________

;;_____________________________PICK_UP________________________________
...
;;_____________________________DROP________________________________
...
to drop-move
  ...
  [
    ...
    [
      ...
      ifelse last-seen = max-memory
      [
        ifelse density >= 4
        [set reward 1]
        [set reward 0]

        if density = 0 [set reward -1]
      ]
      [set reward 0]
...
;;_____________________________MOVE________________________________
to hold-move
 ...
 move
  ...
  [
    if patch-under = 0
    [
      if last-seen = max-memory [set reward -1]
      if last-seen < max-memory [set reward 1]
    ]
...
to update-seen
  ...
  ifelse pcolor != black [set patch-under 1] [set patch-under 0]

  if color != white [set density densityhold_compute]

  if color = white [
    set density density_compute
    if pcolor = black [set density 0]
  ]

  if densityhold_compute >= 1 and actionperformed = "move"
  [
...
;;________________________REWARDS_________________END_STATE__________________________________
...
to-report isEndState
  ...
  if (ticks > episode-length) [report true]
  report false
end
...
;;________________________DENSITY_FUNCTIONS___________________________________

to-report density_compute
  report ((test 1 1) + (test 0 1) +
    (test -1 1) + (test -1 -1)+
    (test 1 0) + (test 1 -1) +
    (test 0 -1) + (test -1 0) + (test 0 0))
end

to-report test [x y]
  ifelse ([pcolor] of (patch-at x y) = pcolor)
  [report 1] [report 0]
end


to-report densityhold_compute
  report ((test2 1 1) + (test2 0 1) +
    (test2 -1 1) + (test2 -1 -1)+
    (test2 1 0) + (test2 1 -1) +
    (test2 0 -1) + (test2 -1 0))
end
...
to-report test2 [x y]
  ifelse ([pcolor] of (patch-at x y) = color)
  [report 1] [report 0]
end
