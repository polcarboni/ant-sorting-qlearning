...
to setup
  ...
  ask Ants [
    ...
    qlearningextension:state-def-extra ["last-seen"] [bla] ;;["xcor" "ycor"]
...
to move
  if (pcolor = color) or (density > 0) [set last-seen (last-seen + 1)]
...
to-report rewardFunc
  ...
  if (last-seen = 1) [set reward 3]
  if (last-seen = 2) [set reward 1]
  if (last-seen = 3) [set reward 0]
  if (last-seen = 4) [set reward -1]
  if (last-seen = 5) [set reward -2]
