...
to setup
  ...
  ask Ants [
    ...
    qlearningextension:action-selection "e-greedy" [0.15 0.9]
    qlearningextension:learning-rate 0.9
    qlearningextension:discount-factor 0.1
...
to go
  ...
  ask Ants [
    ...
    if verbose [
      ...
      print "Hold - Patch - Memory"
      print (word "  " hold "      " patch-under "       " last-seen )
      print " "
...
to pick-up-move
  ...
  if hold = 0 and walk-counter = 0 and density < 4
  [
    if patch-under = 0 [set reward 0]   ;h0 p0
    if patch-under = 1
    [
...
to drop-move
  ...
  [
    ...
    [
      ...
      if last-seen = 5 [set reward 1]
      if last-seen = 4 [set reward 0]
      if last-seen = 3 [set reward 0]
      if last-seen = 2 [set reward 0]
      if last-seen = 1 [set reward 0]
      if last-seen = 0 [set reward 0]
...
to update-seen
  ...
  if density2 >= 1 and actionperformed = "move"
  [
